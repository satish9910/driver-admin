...
const InvoicePreview = ({ order, onClose }) => {
  ...
  const vendor = order.orderItems[0]?.vendor || {
    ...
    address: "Plote no 1, Dwarka sector 1, New delhi, New Delhi, Delhi, India, PIN: 110070",
  ...
  const billingAddress = {
    name: `${order.address.fristname || ''} ${order.address.lastname || ''}`.trim() || "Customer",
    address: [
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-6xl max-h-[90vh] overflow-auto">
        ...
        <div ref={invoiceRef} className="p-8 bg-white" style={{ width: "210mm", minHeight: "297mm", margin: "0 auto" }}>
          ...
          <div className="grid grid-cols-2 gap-8 mb-4">
            <div>
              ...
              <div className="mt-4 text-sm">
                ...
                <p><span className="font-bold">State:</span> {vendorState}</p>
              </div>
            </div>
            <div className="text-sm">
                <div className="mb-4">
                    ...
                    <p><span className="font-bold">State:</span> {customerState}</p>
                </div>
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={index} className="border-t border-gray-300">
                  ...
                  <td className="p-1 text-right">₹{item.taxAmount.toFixed(2)}</td>
                  <td className="p-1 text-right font-bold">₹{item.totalAmount.toFixed(2)}</td>
                </tr>
              ))}
          ...
          <div className="mt-8 text-sm">
            <p><span className="font-bold">Payment Mode:</span> {order.paymentMode === "COD" ? "Cash on Delivery" : order.paymentMode}</p>
            <p><span className="font-bold">Order Status:</span> {order.orderStatus}</p>
            <p className="mt-2 font-semibold">GST Type: {isSameState ? "CGST + SGST (Intra-State)" : "IGST (Inter-State)"}</p>
          </div>
